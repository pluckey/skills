 ...
 *
 * Checks:
 *   FAIL (P1-P11): Referential integrity, structural integrity, fossil detection, wave consistency
...
for (const file of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (headingMatch) {
      ...
      currentId = id;
      currentMeta = { type, title: title.trim(), centerLink: null, traces: [], depends: [], files: [], wave: null, file, line: i + 1 };
      continue;
    }
    ...
    // Match files: > **Files:** path1, path2
    const filesMatch = line.match(/^>\s*\*\*Files:\*\*\s*(.+)/);
    if (filesMatch && currentId) {
      currentMeta.files = filesMatch[1].split(',').map(s => s.trim()).filter(Boolean);
      continue;
    }

    // Match wave: > **Wave:** 2
    const waveMatch = line.match(/^>\s*\*\*Wave:\*\*\s*(\d+)/);
    if (waveMatch && currentId) {
      currentMeta.wave = parseInt(waveMatch[1], 10);
      continue;
    }
...
// P10: Wave consistency — same-wave file overlap
// Preamble: if ANY task has a Wave field, ALL tasks must
const tasksWithWave = tasks.filter(([, meta]) => meta.wave !== null);
const tasksWithoutWave = tasks.filter(([, meta]) => meta.wave === null);

if (!isExpressMode && tasksWithWave.length > 0) {
  if (tasksWithoutWave.length > 0) {
    fails.push({
      check: 'P10',
      message: `Mixed wave assignments: ${tasksWithWave.length} tasks have Wave, ${tasksWithoutWave.length} do not. If any task has Wave, all must.`,
    });
  } else {
    // Group tasks by wave
    const waveGroups = new Map();
    for (const [id, meta] of tasks) {
      if (!waveGroups.has(meta.wave)) waveGroups.set(meta.wave, []);
      waveGroups.get(meta.wave).push({ id, files: meta.files });
    }

    // Check for file overlap within each wave
    for (const [wave, waveTasks] of waveGroups) {
      for (let i = 0; i < waveTasks.length; i++) {
        for (let j = i + 1; j < waveTasks.length; j++) {
          const overlap = waveTasks[i].files.filter(f => waveTasks[j].files.includes(f));
          if (overlap.length > 0) {
            fails.push({
              check: 'P10',
              message: `Same-wave file overlap: ${waveTasks[i].id} and ${waveTasks[j].id} (wave ${wave}) both declare: ${overlap.join(', ')}`,
              ids: [waveTasks[i].id, waveTasks[j].id],
              wave,
              files: overlap,
            });
          }
...
// P11: Backward wave dependency — task in wave N depends on task in wave M where M >= N
if (!isExpressMode && tasksWithWave.length > 0 && tasksWithoutWave.length === 0) {
  for (const [id, meta] of tasks) {
    if (meta.wave === null) continue;
    for (const depId of meta.depends) {
      const depMeta = elements.get(depId);
      if (depMeta && depMeta.wave !== undefined && depMeta.wave !== null && depMeta.wave >= meta.wave) {
        fails.push({
          check: 'P11',
          message: `Backward wave dependency: ${id} (wave ${meta.wave}) depends on ${depId} (wave ${depMeta.wave})`,
          id,
          depId,
          wave: meta.wave,
          depWave: depMeta.wave,
        });
      }
    }
  }
...
const result = {
  ...
  counts: {
    ...
    tasks: tasks.length,
    edges: edges.length,
    waves: tasksWithWave.length > 0 ? Math.max(...tasksWithWave.map(([, m]) => m.wave)) : 0,
  },
  fails,
  flags,
  pass: fails.length === 0,
