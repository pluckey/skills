...
Fill inputs: whiteboard content (post-approval), center sentence.

Launch a sub-agent (separate from the whiteboard agent).

→ Consult `formats.md § Destroyer Verdict Format` for the expected output structure.

**If PASS**: proceed to requirements roundtable.
**If BLOCK**: present the verdict to the user. Options: (a) re-launch whiteboard with reframe, (b) amend the whiteboard, (c) override the Destroyer (logged). Re-run Destroyer after any fix.
